            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
                ...
                @if (count($browse['dirs']) > 0)
                    ...
                    >
                        ...
                        <p class="mb-3 rounded-lg border border-amber-200/70 bg-amber-50/90 px-2 py-2 text-[11px] text-amber-950 dark:border-amber-800 dark:bg-amber-950/50 dark:text-amber-100/95">Si la página <strong>tarda mucho o parece trabada</strong>: es normal en carpetas grandes o RaiDrive lento — no cierres el navegador hasta que Laravel termine la redirección. El servidor usa tiempo de ejecución ilimitado para esta petición; con miles de vídeos y <code class="font-mono">TMDB_AUTO_POSTER_ON_IMPORT</code>, el recursivo <strong>no</strong> llama a TMDB salvo que actives <code class="rounded bg-black/15 px-0.5 font-mono">RAIDRIVE_IMPORT_RECURSIVE_ENRICH_TMDB=true</code> (evita cargas eternas).</p>
                        <div class="mb-3">
