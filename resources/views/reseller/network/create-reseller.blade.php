<x-panel-layout title="Nuevo revendedor en tu red">
    <p class="mb-6 text-sm text-slate-600 dark:text-slate-400">Créditos disponibles: <strong>{{ $balance }}</strong>. Lo que asignes se descuenta de tu cuenta y pasa al nuevo revendedor.</p>

    <form method="POST" action="{{ route('reseller.network.resellers.store') }}" class="max-w-xl space-y-4 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        @csrf
        <div>
